...
<nav class="navbar navbar-inverse fixed-top" id="sidebar-wrapper" role="navigation">
    <ul class="nav flex-column sidebar-nav">
       ...
       <!-- Management Section -->
<li class="nav-item {{ request()->routeIs(['roles.*', 'permissions.*', 'admin.templates.*']) ? 'management-active' : '' }}">
    <a class="nav-link {{ request()->routeIs(['roles.*', 'permissions.*', 'admin.templates.*']) ? '' : 'collapsed' }}"
        data-bs-toggle="collapse" href="#managementSubmenu" role="button"
        aria-expanded="{{ request()->routeIs(['roles.*', 'permissions.*', 'admin.templates.*']) ? 'true' : 'false' }}"
        aria-controls="managementSubmenu">
        <i class="fa fa-cogs mr-2"></i>Management
    </a>
    <div class="collapse {{ request()->routeIs(['roles.*', 'permissions.*', 'admin.templates.*']) ? 'show' : '' }}"
